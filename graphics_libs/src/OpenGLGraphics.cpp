...
void OpenGLGraphics::drawText(const std::string& text, int x, int y, const Color& color, float scale) {
    ...
    for (char c : text) {
        if (c == '\n') {
            currentX = x;
            y += static_cast<int>(16 * scale);
        } else {
            renderBitmapChar(c, currentX, y, color, scale);
            currentX += static_cast<int>(12 * scale);
        }
    }
}

void OpenGLGraphics::renderBitmapChar(char c, int x, int y, const Color& color, float scale) {
    // Simple bitmap font rendering - just draw rectangles for now
    // This is a very basic implementation
    
    int charWidth = static_cast<int>(8 * scale);
    int charHeight = static_cast<int>(12 * scale);
    
    // For simplicity, just draw a rectangle for each character
    // In a full implementation, you'd have proper bitmap font data
    if (c != ' ') {
        drawRectangle(x, y, charWidth, charHeight, color);
    }
}
